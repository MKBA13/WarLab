...
export const play = (
  ...
  const tempObj = audioMap[type];
    ...
    if (typeof tempObj === 'string') {
      audioName = tempObj;
    } else if (tempObj && typeof tempObj === "object" && increase !== null) {
        audioName = tempObj[increase ? 'up' : 'down'] || "";
    } else {
      audioName = "";
    }

    if(!audioName) return;  //if there is no audio, then do not continue to avoid errors

  const audio = audios[audioName];
  if (!audio) {
    return; // If the audio doesn't exist, don't try to play it
  }
